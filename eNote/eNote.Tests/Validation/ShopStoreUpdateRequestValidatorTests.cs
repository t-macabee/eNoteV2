...
public sealed class ShopStoreUpdateRequestValidatorTests
{
    private readonly ShopStoreUpdateRequestValidator _validator = new();
    ...
    public void Validate_RejectsInvalidPhoneNumberFormat(string phoneNumber)
    {
        ...
        Assert.Contains(result.Errors, e => e.PropertyName == nameof(ShopStoreUpdateRequest.PhoneNumber) && e.ErrorMessage == Messages.PhoneNumberFormat);
    }
    ...
    public void Validate_RejectsPhoneNumberExceedingMaximumLength()
    {
        ...
        Assert.False(result.IsValid);
        Assert.Contains(result.Errors, e => e.PropertyName == nameof(ShopStoreUpdateRequest.PhoneNumber) && e.ErrorMessage == Messages.PhoneNumberTooLong);
    }

    private static ShopStoreUpdateRequest ValidRequest() => new()
    {
