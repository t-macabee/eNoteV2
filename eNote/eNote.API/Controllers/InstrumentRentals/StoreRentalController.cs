...
namespace eNote.API.Controllers.InstrumentRentals
{
    ...
    public sealed class StoreRentalController(IRentalQueryService queryService, IRentalCommandService commandService) : CoreController
    {
        ...
        {
            var dto = await commandService.ApproveAsync(id, CurrentUserId, response);
            return Ok(dto);
        }
        ...
        {
            var dto = await commandService.RejectAsync(id, CurrentUserId, response);
            return Ok(dto);
        }
        ...
        {
            var dto = await commandService.PickupAsync(id, CurrentUserId, response);
            return Ok(dto);
        }
        ...
        {
            var dto = await commandService.CompleteAsync(id, CurrentUserId, response);
            return Ok(dto);
        }
        ...
        {
            var dto = await commandService.ReturnEarlyAsync(id, CurrentUserId, response);
            return Ok(dto);
        }
    }
}
