...
public class ENoteContext(DbContextOptions<ENoteContext> options, IClock clock, ICurrentActor actor) : IdentityDbContext<AppUser, AppRole, int>(options), IAppDbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        // Lecture.Version is a byte[] rowversion concurrency token, generated by the
        // database on every insert/update on relational providers (SQL Server maps it
        // to the native `rowversion` type). The InMemory provider used by tests has no
        // such mechanism and otherwise rejects the property as a missing required
        // value, so give it an explicit generator there only.
